...
public class GalaxyDriver extends AbstractDriver{
			...
			if (argv == null) { //Terminaos el fichero
				...
				switch(opc) {
					case 0:
						break;
					case 1:
						if (argv.length < 4) _msg_error_param_insuf();
						else {
							g = create_galaxy_full(argv[1], Integer.parseInt(argv[2]), Integer.parseInt(argv[3]));
						}
						break;
					...
					case 4:	
						if (argv.length < 3) _msg_error_param_insuf();
							...
							set_galaxy_size(g,  Integer.parseInt(argv[1]), Integer.parseInt(argv[2]));
						}
						break;
					...
					case 6:
						if(g != null) Console.print(g.getName());
						break;
	...
	private static void _menu() {
		...
		menu.add("Galaxy(String name, int x, int y) : Planet"); //1
		menu.add("Galaxy() : Planet");
